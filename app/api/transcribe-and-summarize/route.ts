...
async function callAnalysisWorker(
  ...
  const promptLines = [
    ...
    "- [구조] 이 최상단 콜아웃 다음으로, 전체 내용을 대주제 단위로 나누어 \"## 1. 대주제명\", \"## 2. 대주제명\"처럼 " +
      ...
      "설명·배경지식·세부 내용은 기본적으로 평범한 문단이나 글머리 기호(- 또는 •) 리스트로 작성하세요.",
    "- [시험 출제 신호 감지 — 절대 누락 금지] 교수가 \"시험에 나온다\", \"무조건 출제된다\", \"별표 쳐라\", " +
      '"이거 매우 중요하다"처럼 시험 출제 가능성을 직접적으로 언급하거나, 반복해서 강조하거나 어조를 바꿔 ' +
      "주의를 환기하는 등 같은 취지를 간접적으로 암시하는 구간이 있다면, 강의 전체에서 단 한 곳도 빠짐없이 " +
      "찾아내세요. 해당 내용은 다른 콜아웃이나 일반 텍스트와 뚜렷이 구별되도록 아래 형식을 정확히 그대로 " +
      "사용해 표시하세요 (대괄호 안 문구와 굵게 표시 포함):\n" +
    ...
    "- [비교 표 필수] 성적 평가 비율, 과제 제출 일정, AI 활용 가이드라인처럼 서로 비교 가능한 항목이 3개 이상 " +
      ...
      "형식, 구분선 행 포함)로 작성하세요.",
    "- [부가 정보는 토글로] 교수 소개, 시험과 무관한 일회성 아이스브레이킹 활동처럼 본문 흐름과 직접 관련 없는 부가" +
      "참고 정보는 아래 예시와 정확히 같은 형식으로 감싸서, 본문이 번잡해지지 않도록 하세요 (summary에는 짧은 " +
      "제목만 넣고, <details>/<summary>/</details> 태그는 반드시 각각 단독 줄에 작성):\n" +
      "  <details>\n" +
