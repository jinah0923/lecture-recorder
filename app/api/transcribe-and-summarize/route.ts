...
export async function POST(request: Request) {
  ...
  const systemInstruction = [
    ...
    "transcript는 반드시 오디오 전체 분량에 대한 100% 완전한 받아쓰기여야 합니다. 오디오가 길다는 이유로 일부 구간을 " +
      "생략, 압축, 요약하는 것은 절대 허용되지 않습니다.",
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        // A literal 8192 would actually *guarantee* truncation for exactly
        // the case this is meant to fix: a 30+ minute lecture's verbatim
        // transcript alone can exceed 8192 tokens before the summary/
        // lectureNote/checklist fields even start. 65536 is the ceiling for
        // current Gemini Flash-tier models — the API clamps to whatever the
        // model actually supports, so this is safe even if the true max for
        // this specific model differs.
