    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
          <h2 id="notion-export-modal-title" className="text-sm font-semibold text-zinc-900">
            🗂️ 노션으로 내보내기
          </h2>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {status === "success" ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm font-semibold text-zinc-900">노션 페이지에 강의노트를 추가했어요!</p>
              {resultUrl && (
                <a
                  href={resultUrl}
                  target="_blank"
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-zinc-600">Notion API 토큰</label>
                <div className="flex gap-1.5">
                  <input
                    ...
                    placeholder="ntn_..."
                    className="flex-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-300"
                  />
                  <button
                    type="button"
                    onClick={() => setShowToken((current) => !current)}
                    className="shrink-0 rounded-lg border border-zinc-200 px-3 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100"
                  >
                    {showToken ? "숨기기" : "보기"}
                  </button>
                </div>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-zinc-600">대상 노션 페이지 URL</label>
                <input
                  value={targetInput}
              ...
              <div className="rounded-lg bg-zinc-50 px-3 py-2 text-[11px] leading-relaxed text-zinc-500">
              ...
              {status === "error" && errorMessage && (
                <p className="rounded-lg bg-red-50 px-3 py-2 text-xs leading-relaxed text-red-600">{errorMessage}</p>
              )}
            </div>
          )}
        </div>
        ...
        {status !== "success" && (
          <div className="flex items-center justify-end gap-2 border-t border-zinc-100 px-5 py-3">
            <button
              type="button"
              onClick={onClose}
              className="rounded-full border border-zinc-200 px-4 py-1.5 text-sm font-medium text-zinc-600 transition hover:bg-zinc-100"
            >
              취소
            </button>
            <button
