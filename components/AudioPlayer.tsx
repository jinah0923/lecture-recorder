  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">오디오 플레이어</p>
        <button
          type="button"
          onClick={() => downloadBlob(fileName, audioBlob)}
          className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-700 transition hover:bg-zinc-100"
        >
      ...
      <div className="mt-3 flex items-center justify-end gap-1.5">
        <span className="mr-1 text-xs text-zinc-400">배속</span>
        {SPEEDS.map((value) => (
          <button
            key={value}
            type="button"
