  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-zinc-900">이번 주 해야 할 일</h2>
          {total > 0 && (
        ...
        <ul className="flex flex-col gap-2">
          {visibleItems.map((item) => (
            <li
              ...
              className="flex items-start gap-3 rounded-xl bg-zinc-50 px-3 py-2.5"
            >
              <input
                type="checkbox"
                checked={item.done}
              ...
              <div className="min-w-0 flex-1">
                <p className={item.done ? "text-sm text-zinc-400 line-through" : "text-sm text-zinc-700"}>
                  {item.text}
                </p>
                <button
                  type="button"
                  onClick={() => onNavigateToSession(item.sessionId)}
                  className="mt-1 inline-flex max-w-full items-center gap-1 rounded-full border border-zinc-200 bg-white px-2 py-0.5 text-[11px] text-zinc-500 transition hover:border-indigo-200 hover:text-indigo-600"
                >
